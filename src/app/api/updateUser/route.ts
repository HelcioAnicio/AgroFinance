...
import { getCurrentFarmContext, hasFarmPermission } from '@/lib/tenant';
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const farmContext = await getCurrentFarmContext();
    ...
    if (Object.keys(data).length === 0 && !farmName) {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      if (
        farmContext &&
        farmName &&
        hasFarmPermission(farmContext.role, 'manage_farm')
      ) {
